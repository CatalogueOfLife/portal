...
export const sources = [...metadata.sources].sort((a, b) => (a.alias || '').localeCompare(b.alias || ''));
...
<div id="citation" set:html={metadata.current.citation} />
...
<div class="sources">
  {sources.map((src) => (
    <div>
      <span set:html={src.citation} /> <a href={`/data/dataset/${src.key}`}><img style={{ height: '12px', opacity: 0.6 }} src="/images/link.png" /></a>
    </div>
  ))}
</div>
